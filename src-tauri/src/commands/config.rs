...
pub async fn list_configs(state: State<'_, AppState>) -> Result<Vec<TaxConfig>, String> {
    ...
    let configs = stmt.query_map([], row_to_config)
        .map_err(|e| e.to_string())?
...
#[tauri::command]
pub async fn save_config(config: serde_json::Value, state: State<'_, AppState>) -> Result<TaxConfig, String> {
    let id = Uuid::new_v4();
    let now = Utc::now();
    {
        let guard = state.db.lock().map_err(|e| e.to_string())?;
        ...
        conn.execute("UPDATE tax_config SET is_active=0", []).map_err(|e| e.to_string())?;
        conn.execute(
...
pub async fn export_config_json(id: String, state: State<'_, AppState>) -> Result<String, String> {
    ...
    let cfg = conn.query_row(
        ...
        params![id], row_to_config,
    ).map_err(|e| e.to_string())?;
    serde_json::to_string_pretty(&cfg).map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn import_config_json(json: String, state: State<'_, AppState>) -> Result<TaxConfig, String> {
    // Validate structure
    let _: TaxConfig = serde_json::from_str(&json)
