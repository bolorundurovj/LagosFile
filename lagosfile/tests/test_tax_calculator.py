...
# Property 25: Tax calculation accuracy
@given(
...
def test_tax_calculation_accuracy(taxable_income, cgt_proceeds, cgt_gain, allowances):
    """Test that tax calculations are accurate and consistent"""
...
# Property 26: Tax calculation consistency
@given(
    st.floats(min_value=0, max_value=10000000),
    st.floats(min_value=0, max_value=1000000),
    st.floats(min_value=0, max_value=1000000),
...
def test_tax_calculation_consistency(
    taxable_income, cgt_proceeds, cgt_gain, allowances
):
    """Test that tax calculations are consistent between sync and async methods"""
    # Create calculator instance
    calculator = TaxCalculator()
...
# Property 27: Allowance calculation accuracy
@given(
    st.dictionaries(
        st.text(min_size=1, max_size=20),
        st.floats(min_value=0, max_value=1000000),
...
def test_allowance_calculation(allowances, rent_relief_cap):
    """Test that allowance calculations are accurate"""
...
# Property 28: Income tax calculation accuracy
@given(
    st.floats(min_value=0, max_value=10000000),
    st.lists(
        st.tuples(
...
def test_income_tax_calculation(income, bands):
    """Test that income tax calculations are accurate"""
...
# Property 29: CGT calculation accuracy
@given(
    st.floats(min_value=0, max_value=10000000),
    st.floats(min_value=0, max_value=10000000),
    st.floats(min_value=0, max_value=10000000),
...
def test_cgt_calculation(proceeds, gain, threshold):
    """Test that CGT calculations are accurate"""
