...
class ExportModal(ft.Container):
    ...
    def __init__(
        self, page: ft.Page, filing_export: FilingExport | None = None
    ) -> None:
        super().__init__()
        self._page = page
        self.expand = True
        ...
        try:
            if fmt == "pdf":
                data = self._export_engine.export_pdf(
                    self.filing_export, include_attachments=include_attachments
                )
                filename = (
                    f"lagosfile_{self.filing_export.yoa}_{self.filing_export.tin}.pdf"
                )
                self._save_bytes(data, filename)
            elif fmt == "csv":
                data = self._export_engine.export_csv(self.filing_export)
                filename = (
                    f"lagosfile_{self.filing_export.yoa}_{self.filing_export.tin}.csv"
                )
                self._save_text(data, filename)
            elif fmt == "json":
                data = self._export_engine.export_json(self.filing_export)
                filename = (
                    f"lagosfile_{self.filing_export.yoa}_{self.filing_export.tin}.json"
                )
                self._save_text(data, filename)
